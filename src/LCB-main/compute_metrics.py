...
dict_path = 'src/language-confusion-main/words'  # downloaded from https://gist.githubusercontent.com/wchargin/8927565/raw/d9783627c731268fb2935a731a618aa8e95cf465/words
...
def compute_metrics(completions: Iterable[str], lang: str) -> dict[str, float]:
    ...
    for completion in completions:
        ...
        if lines:
            ...
            # line language distribution, added 2oct.
            # line_dist_arr = [langid(line) for line in lines]
            # distribution = collections.Counter(line_dist_arr)
            # # Total number of elements in the array
            # total_elements = len(line_dist_arr)
            #
            # # Calculate probability for each element
            # line_dist = {key: value / total_elements for key, value in distribution.items()}
            # line_ids_list.append(line_dist)
    ...
    # can lingua and higher score remedy this problem?
...
if __name__ == '__main__':
    ...
    for file in os.listdir(datadir):
        if file.endswith(".csv"):
            ...
            with open(os.path.join(datadir, file)) as csv_file:
            ...
            for (task, model), outputs_ in itertools.groupby(outputs, key=group_key):
                ...
                for (source, lang), metrics in all_metrics.items():
                    lpr = f"{metrics['lpr']:.2}"
                    wpr = f"{metrics['wpr']:.2}" if 'wpr' in metrics else np.nan
                    generation_settings.append(task)
                    models.append(model)
    ...
    df.to_csv("datasets/prompts_language_confusion/results/reproduction_pass_rates_results.csv", index=False)
