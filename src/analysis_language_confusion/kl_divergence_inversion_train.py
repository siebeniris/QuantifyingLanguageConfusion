...
def get_lang2lang_matrix_for_inversion_train(df, lingual="crosslingual"):
    if lingual == "crosslingual":
        df = df[df["eval_in_training"] == False]
    elif lingual == "monolingual":
        df = df[df["eval_in_training"] == True]

    df = df.dropna(subset=["pred_langs_dist"])
    df["pred_langs_dist"] = df["pred_langs_dist"].apply(literal_eval)
    df["training_langs"] = df["training_langs"].apply(literal_eval)

    lang2lang_dist = defaultdict(dict)
    for lang, pred_langs, train_langs in zip(df["eval_lang"], df[f"pred_langs_dist"], df["training_langs"]):
        if lang == "ara":
            lang = "arb"
        if lang == "zho":
            lang = "cmn"
        ...
        for train_lang in train_langs:
            ...
            for pred_lang, v in pred_langs.items():
                ...
                if pred_lang not in lang2lang_dist[train_lang]:
                    lang2lang_dist[train_lang][pred_lang] = list()

                if pred_lang in train_langs:
                ...
                # maybe change this????
                lang2lang_dist[train_lang][pred_lang].append(reweighted)
...
def main():
    for level in ["line", "word"]:
        ...
        for lingual in ["crosslingual", "monolingual", "all"]:
            df = pd.read_csv(
                f"results/inversion_language_confusion/{level}_level/dataframes/multi_{level}_{lingual}_weighted_entropy.csv")

            outputfolder = f"results/inversion_language_confusion/kl_divergence/{level}/{lingual}/train"
            if not os.path.exists(outputfolder):
                os.makedirs(outputfolder)

            outputfolder_colex2lang = os.path.join(outputfolder, "colex2lang")
            if not os.path.exists(outputfolder_colex2lang):
                os.makedirs(outputfolder_colex2lang)

            outputfolder_ostling = os.path.join(outputfolder, "ostling")
            if not os.path.exists(outputfolder_ostling):
                os.makedirs(outputfolder_ostling)

            l2l = get_lang2lang_matrix_for_inversion_train(df, lingual=lingual)

            for file in os.listdir("datasets/colex2lang_sim"):
                if file.endswith(".csv"):
                    lang_typ_file = os.path.join("datasets/colex2lang_sim", file)
                    get_kl_divs(l2l, lang_typ_file, outputfolder_colex2lang, "colex2lang")

            for file in os.listdir("datasets/ostling_lang2sim"):
                if file.endswith(".csv"):
                    lang_typ_file = os.path.join("datasets/ostling_lang2sim", file)
                    get_kl_divs(l2l, lang_typ_file, outputfolder_ostling, "ostling")
